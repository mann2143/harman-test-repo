...
if [ -z "$1" ]; then
  echo "No commit message file provided. Exiting."
  exit 1
fi

# Check if the commit message file is empty
if [ -s "$1" ]; then
  echo "Commit message already exists. Skipping prompt."
  exit 0
fi
...
# Save the commit message to the file passed as an argument
